    ...
    const NOTIF_SESSIONS = [
        "f55f0cb6-bee2-46bf-b8c0-a1f550b5699b",
    ...
    function buildMessage(data, bill = null, opt = {}) {
        ...
        //     lines.push(`${bold(data.keterangan)}`);
        // }

        if (reportAwal || reportDate) {
            // lines.push("");
            lines.push(`📌 📊 *Info Report*`);
            if (reportAwal) lines.push(`  • Report Awal: ${bold(reportAwal)}`);
            if (reportDate) lines.push(`  • Tanggal Report: ${bold(reportDate)}`);
            if (aksesTanggal) lines.push(`  • Tanggal Akses: *${aksesTanggal}*`);        }

        if (reportMessage) {
            // lines.push("");
            lines.push(`📌 ⚠️ *STATUS AWAL INFO*`);
            lines.push(`  ${bold(String(reportMessage).toUpperCase())}`);
        }
        ...
        if (alamat) {
            if (map) {
                lines.push(`  • 📍 Lokasi: ${alamat} (${map})`);
            } else {
                lines.push(`  • 📍 Lokasi: ${alamat}`);
            }
        } else if (map) {
            lines.push(`  • 📍 Lokasi: ${map}`);
        }
